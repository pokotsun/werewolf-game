...
class VillageGrpcService(
    ...
    override fun getCurrentVillageUsers(
        ...
        val villageAndUsers = getCurrentVillageUsersUseCase.invoke(
            request.villageId,
        ...
        // レスポンスを作成
        val currentUsersResponseList = villageAndUsers.second.map { user ->
            CurrentUserResponse.newBuilder()
                .setUserName(user.name)
                .build()
        }
        val getCurrentVillageUsersResponse = GetCurrentVillageUsersResponse.newBuilder()
            .setVillageId(villageAndUsers.first.id.value.toString())
        ...
        responseObserver.let { r ->
            ...
            // 村が募集中でない場合は処理を終了する
            if (!villageAndUsers.first.isRecruited) {
                r.onCompleted()
            }
        }
    }
