...
func main() {
	p := tea.NewProgram(preventQuitInitialModel(), tea.WithFilter(filter))
...
type preventQuitModel struct {
	textarea   textarea.Model
	help       help.Model
	keymap     preventQuitKeymap
	saveText   string
	hasChanges bool
	quitting   bool
}

type preventQuitKeymap struct {
	save key.Binding
	quit key.Binding
}

func preventQuitInitialModel() preventQuitModel {
	ti := textarea.New()
...
func (m preventQuitModel) Init() tea.Cmd {
	return textarea.Blink
}

func (m preventQuitModel) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	if m.quitting {
...
func (m preventQuitModel) updateTextView(msg tea.Msg) (tea.Model, tea.Cmd) {
	var cmds []tea.Cmd
	var cmd tea.Cmd
...
func (m preventQuitModel) updatePromptView(msg tea.Msg) (tea.Model, tea.Cmd) {
	switch msg := msg.(type) {
	case tea.KeyMsg:
...
func (m preventQuitModel) View() string {
	if m.quitting {
		if m.hasChanges {
